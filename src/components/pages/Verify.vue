<template>
  <div style="padding: 2%;text-align: right;">
    <br><br>
    <h3 style="text-align:center">تایید حساب کاربری</h3><br><br>
    <div class="card navdark">
      <div class="card-body">
        ...
        <h2 style="text-align:center;width:100%;text-align:center" class="btn btn-info"
          v-if="mobilev && bankv && emailv && idv && rulev && melliv && accountv && acceptv && !coinv"> اطاعات شما تکمیل
          شده و حساب شما پس از بررسی کارشناسان ما تایید خواهد شد </h2>
        <br><br>
      </div>
    </div><br><br>
    ...
    <div hidden style="margin-bottom: 50px" class="b-ver onec" id="idcard card navdark">
      <div class="card-body">
        <h4 id="idtitle">ثبت اطاعات هویتی</h4><br><br><br>
        <b-input style="width:74%;float:left;height:40px" type="text" class="mellinum" v-model="first_name" id="mell"
          placeholder="" /><label style="width:25%;height:40px"> نام</label>
        <b-input style="width:74%;float:left;height:40px" type="text" class="mellinum" v-model="last_name" id="mell"
          placeholder="" /><label style="width:25%;height:40px"> نام خانوادگی</label>
        <b-input style="width:74%;float:left;height:40px" type="text" class="mellinum" v-model="address" id="mell"
          placeholder="" /><label style="width:25%;height:40px"> آدرس</label>
        <b-input style="width:74%;float:left;height:40px" maxlength="10" minlength="10" type="tel" class="mellinum"
          v-model="postal" id="mell" placeholder="" /><label style="width:25%;height:40px"> کد پستی</label>

        <br> <br>
        <a href="javascript:void(0)" @click="submit()" id="ebtn" class="small btn btn-dark" style="float:left">ثبت
          اطاعات</a>
    ...
    <div hidden style="margin-bottom: 50px" class="b-ver  threec card navdark" id="mellicard">
      <div class="card-body">
        <div style="text-align:center" v-if="!melliv">
          ...
          <b-form-group label=" "><br><br><br>
            <b-input style="width:74%;float:left" maxlength="10" minlength="10" type="tel" class="mellinum"
              v-model="mellinum" id="mell" placeholder="" /><label style="width:25%" for="mell">شماره کارت ملی</label>
            <div style="clear:both"></div><br>
            <input type="file" id="filemelli" />
            <a href="javascript:void(0)" @click="submitmelli()" class=" btn btn-dark" style="margin:20px"> ارسال کارت ملی
              برای تایید</a>

          </b-form-group>
        </div><br><br>
        <div style="text-align:center" v-if="!acceptv">
          ...
          <b-form-group label=" "><br><br><br>
            <div style="clear:both"></div><br>
            <a class="btn btn-dark" target="_blank" href="https://www.amizax.com/img/verify/accept.jpg">نمونه
              تصویر</a><br>
            <input type="file" id="fileaccept" />
            <a href="javascript:void(0)" @click="submitaccept()" class=" btn btn-dark" style="margin:20px"> ارسال برگ
              پذیرش
              مقررات</a>
    ...
    <div hidden style="margin-bottom: 50px" class="b-ver twoc card navdark" id="mobilecard">
      <div class="card-body">
        ...
        <b-form-group v-if="!mobilev" label=" "><br><br>
          ...
          <button @click="mobilesend(); getstatus()" class="small btn btn-dark" id="mbtn"><a
              v-if="timerNum !== 60 && timerNum !== 0">{{ timerNum }}</a> ارسال کد</button>
          <b-input hidden :placeholder="'کد ارسال شده'" v-model="mcode" id="mc" />
          <a hidden @click="mobileverify()" href="javascript:void(0)" class="small btn btn-success" id="mcb">تایید
            شماره
            همراه</a>
        ...
        <b-form-group v-if="!emailv" label="">
          ...
          <button href="javascript:void(0)" @click="emailsend(); getstatus2()" id="ebtn" class="small btn btn-dark"><a
              v-if="timerNum2 !== 60 && timerNum2 !== 0">{{ timerNum2 }}</a> ارسال کد</button>
          <b-input hidden :placeholder="'کد ارسال شده'" v-model="ecode" id="ec" />
          <a hidden @click="emailverify()" href="javascript:void(0)" id="ecb" class="small btn btn-success">تایید آدرس
            ایمیل
          </a>
    ...
    <div hidden style="margin-bottom: 50px; text-align:center" class="b-ver card navdark  fivec" id="acceptcard">
      <div class="card-body">
        <form @submit.prevent="submitcard()" action="">
          <div id="addcard" header="اضافه کردن حساب بانکی" header-tag="h6" class="mb-4">
            <fieldset class="demo-vertical-spacing-sm">
              <b-form-group>
                <b-input maxlength="16" required style="text-align:center" minlength="16" type="text" class="cardnum"
                  v-model="cardnumber" @change="checknum()" @focusout="checknum2()" @focus="checkback()"
            ...
            <fieldset class="demo-vertical-spacing-sm">
              <div class="input-group mb-3" style="direction:ltr">
                ...
                <input type="text" required minlength="24" maxlength="24" v-model="shebanumber" style=";text-align:center"
                  class="form-control" placeholder="شماره شبا" aria-label="Username" aria-describedby="basic-addon1">
              </div>
            </fieldset>
            <fieldset class="demo-vertical-spacing-sm">
              <b-form-group>
                <b-input style="text-align:center" maxlength="16" minlength="16" type="tel" class="cardnum"
                  v-model="accountnumber" placeholder=" شماره حساب (اختیاری)" />
                <div class="invalid-tooltip">{{ ctool }}</div>
              </b-form-group>
    ...
    <div hidden v-if="!rulev" style="margin-bottom: 50px" class="b-ver card navdark fourc" id="rulecard">
      <div class="card-body">
    ...
    async get_user() {
      await axios
        ...
        .then(response => {
          ...
          this.isphone = response.data.phone
        })
    },
    async mobilesend() {
